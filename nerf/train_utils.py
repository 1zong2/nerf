...
def predict_and_render_radiance(
    ...
    z_vals = z_vals.expand([num_rays, options.nerf[f"{mode}"]["num_coarse"]])

    if options.nerf[f"{mode}"]["perturb"]:
    ...
    (
        ...
        z_vals,
        rd,
        radiance_field_noise_std=options.nerf[f"{mode}"]["radiance_field_noise_std"],
        white_background=options.nerf[f"{mode}"]["white_background"],
    )

    rgb_fine, disp_fine, acc_fine = None, None, None
    if options.nerf[f"{mode}"]["num_fine"] > 0:
        ...
        z_samples = sample_pdf(
            z_vals_mid,
            weights[..., 1:-1],
            options.nerf[f"{mode}"]["num_fine"],
            det=(options.nerf[f"{mode}"]["perturb"] == 0.0),
        )
...
def run_one_iter_of_nerf(
    ...
    batches = get_minibatches(rays, chunksize=options.nerf[f"{mode}"]["chunksize"])
    pred = [
